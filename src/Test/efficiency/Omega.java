    ...
    public void run() {
        ...
        for (int d = 0; d < 1; d++)
        {
            ...
            for (int q = 0; q < 1; q++)
            {
                ...
                {
                    omiga_y = i+1;
                    Parameter.w1 = omiga[i][0];
                    Parameter.w2 = omiga[i][1];
                    Parameter.w3 = omiga[i][2];
                    ...
                    for (int number = 0; number < 1; number++)
                    {
                        ...
                        for (int a = 0; a < InitialVNode.size(); a++) {
                            ...
                            for (int b = 0; b < tmp.size(); b++) {
                                ...
                                Root.Upper = (1 - Beta) * Math.atan(Root.UpperMatchingNode.size())
                                        * 2 / Math.PI + Beta * Root.MaxTrust;
                        ...
                        for (int e = 0; e < CanTrans.size(); e++) {
                            ...
                            //计算社交影响力和属性的综合评分;
                            //temp.low = Beta * Math.atan(temp.matches_low.size()) * 2 / Math.PI
                            //+ (1 - Beta) * temp.sumOfTrust_low / temp.matchingPaths_low;

                            if (temp.matchingPaths_low == 0) {
                                temp.low = 0;
                            } else {
                                temp.low2 = (1 - Beta) * Math.atan(temp.matches_low.size()) * 2 / Math.PI
                                        + Beta * temp.sumOfTrust_low / temp.matchingPaths_low;

                                temp.low = (1 - Beta) * Math.atan(temp.matches_low.size()) * 2 / Math.PI
                                        + Beta * temp.sumOfTrust_low / temp.UpperNumOfMatchingPath;
                            }
                            ...
                            if (temp.low != 0) {
                                ...
                                if (Termination(heapMin, CanTrans, MaxIndex) == true && Tmp <= 1)
                                {
                                    ...
                                    for(int T = 0;T<heapMin.size;T++)
                                        System.out.println(heapMin.Heap.get(T).dataVNode.nodeID+" "+heapMin.Heap.get(T).low+" "+heapMin.Heap.get(T).low2+" "+heapMin.Heap.get(T).Upper);
            ...
            try {
                ...
                FileWriter writer = new FileWriter("D:\\Users\\shiqun\\Desktop\\效率\\" + d + "\\Omega" + "_MTK" + ".txt");
